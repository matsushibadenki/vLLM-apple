...
228. `[Next]` Core ML persistent workerからfinal／deep-stack 3本のFP16 payloadをprivate・size-bound・digest-bound transportでHomebrew processへ渡し、コピー回数を計測した上でMLX配列へ変換。実pixel inputによるend-to-end chat vision smokeと長時間memory soakを独立qualification。Audio／embedding／classifierは対応artifactごとに独立probeし、合格用途だけ昇格
