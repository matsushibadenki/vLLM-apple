...
235. `[Next]` end-to-end calibrationへ形状・物体・簡易OCRを追加し、単純色識別以外のvision-language品質境界を測定する。その後、最小mixed-precision配置、Core ML実device割当、copy overhead、長時間memory soakを評価。Audio／embedding／classifierは対応artifactごとに独立probeし、合格用途だけ昇格
