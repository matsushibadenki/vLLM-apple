...
- `[Done]` 配置済みQwen-Image-2.1 revision `b3179ad355be050328e483a9dfdd9e60cd62adfa`を画像生成候補catalog、Diffusers静的pipeline readiness、isolated worker routingへ追加。loadなし検査でDiffusers形式、`QwenImage21Pipeline`、33,131,616,240 bytes、27 files、BF16非量子化、transformer 14.23 GB、text encoder 17.53 GB、VAE等1.35 GBを確認。Qwen Research Licenseは非商用限定。証跡: [Qwen-Image-2.1 readiness](evaluation/qwen-image-2.1-artifact-readiness-2026-09-20.json)
- `[Next]` Qwen-Image-2.1用の隔離runtime環境（Transformers 5.17以上＋`QwenImage21Pipeline`を含むDiffusers）を固定し、M4/32GB向け量子化または逐次module residency artifactを準備してload-before admissionを通す。現行BF16 artifactの全常駐実行は禁止
