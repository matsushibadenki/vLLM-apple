...
243. `[Next]` persistent Core ML vision＋Homebrew MLX language経路をserverのmodel lifecycle／request APIへ統合し、同時chat requestのbackpressure、cancel／timeout、client切断時cleanupを実HTTP経路で検証する。一般runtime昇格はopen-domain品質gateまたはBF16相当precisionの実装まで保留
