...
227. `[Next]` persistent pipelineのfinal `[64,2048]`と3 deep-stack `[64,2048]`を既存Qwen3-VL embedding adapterのshape／順序／graph provenance契約へ結合し、秘密情報を含まないbounded transportでHomebrew vLLM-Metalのvision embedding handoffへ接続。接続後にend-to-end chat vision smokeと長時間memory soakを独立qualification。Audio／embedding／classifierは対応artifactごとに独立probeし、合格用途だけ昇格
