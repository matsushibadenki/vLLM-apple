...
240. `[Next]` persistent vision workerを同一processのまま30分以上連続実行し、全request digest、失敗／timeout／restart数、latency分布、RSS成長、graceful shutdownを評価する。合格後にruntime transport／schedulerのQwen3-VL encoder routeへ接続する。一般runtime昇格はopen-domain品質gateまたはBF16相当precisionの実装まで保留
