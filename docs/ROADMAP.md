...
239. `[Next]` Core ML modelを一度だけloadするpersistent vision workerをruntime transportへ接続し、同一processで30分以上のmemory soak、bounded request queue、timeout／crash時の再起動、graceful shutdownを検証する。Audio／embedding／classifierは対応artifactごとに独立probeし、合格用途だけ昇格
