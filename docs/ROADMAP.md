...
220. `[Next]` 最初のdeep-stack index 5を含むblock 0〜5の6層graphへ拡張し、中間hidden stateをdeep-stack mergerへ接続してmain／deep-stack両出力を数値照合。その後index 11／17、24 block、final merger、既存vLLM-Metal adapter接続、end-to-end latency／memoryを段階的に実機qualification。Audio／embedding／classifierは対応artifactごとに独立probeし、合格用途だけ昇格
