...
5. `[Pending]` 将来のHomebrew candidateが独立した3回のqualificationで同じ証跡を満たした場合だけ、対応version matrixと標準backendへ昇格する。現行0.29.0の単一認定結果を反復実行で水増しせず、次のcandidate release／独立認定機会まで保留する。
...
- `[Pending]` 5%以上改善するmodel固有CPU/Core ML draft＋GPU verifierの実機qualification。Gemma 3 1B CPU＋4B GPUはcorrectnessに合格したが約11.59倍遅く認定しない。配置済みmodelにはGemma 3 1Bより小さく同一tokenizer／vocabularyを持つdraftも、Gemma用Core ML draft artifactも存在しないため、より小さい互換draft、十分大きいGPU verifier、またはCore ML/ANE draft artifactの配置まで保留する。artifact配置後は3 sample以上・出力一致・5%以上改善を満たすprofileだけを標準decode routeへ注入する
...
- `[Done]` Qwen-Image-2512 4-bit MFLUXの現行Mac向けcomponent streaming/offload。encoder／generationを完全に別process化したstaged経路で、positive／negative実prompt→true CFG→実ノイズ20 step→量子化transformer固定層＋RoPE＋2,400 block→256px VAE／PNGまで完走した。160px合格reportを同一artifact・all-normal・cleanup・20-step条件で再検証するevidence-bound promotion gateを使い、256pxは全step pressure normal、最終thermal fair、generation peak MLX 2,182,266,500 bytes、peak RSS 633,749,504 bytes、約478.8秒。PNG digest一致、赤いリンゴ、茎、白系の台面、接地影を実視認でき意味品質に合格した。配置artifactは25,907,123,451 bytes／21 files。従来の一括512×512・20 steps・2 sampleはresident見積り27,917,287,424 bytesでload前停止したため、同じ一括経路を再試行しない。証跡: [256px staged execution](evaluation/qwen-image-2512-mflux-staged-real-prompt-cfg-20step-256px-2026-09-24.json)、[256px visual review](evaluation/qwen-image-2512-mflux-staged-real-prompt-cfg-20step-256px-visual-review-2026-09-24.json)、[generated PNG](evaluation/qwen-image-2512-staged-real-prompt-cfg-20step-256px-2026-09-24.png)
...
- `[Done]` 128×128・2 stepの初回試験でdecode直後のmemory pressure warningを検出し、decoder前の不要weight／中間tensor解放とVAE encoder非保持へ修正した。空きメモリ不足時の再試験はstep 0で安全停止し、十分な空きメモリでの再qualificationにより後続の128px／256px合格へつなげた。失敗reportは改善履歴として保持する。証跡: [128px pressure warning](evaluation/qwen-image-2512-mflux-streamed-2step-128px-synthetic-2026-09-22.json)、[cleanup後のadmission stop](evaluation/qwen-image-2512-mflux-streamed-2step-128px-synthetic-cleanup-2026-09-22.json)
- `[Done]` decoder前の不要weight／中間tensor解放とVAE encoder非保持後、空きメモリ12.5 GBから128×128・2 stepを再qualification。2/2 step・120/120 block、有限`[1,3,1,128,128]`、pressure全件normal／thermal nominal、peak MLX 1,188,085,938 bytes、process peak RSS 565,231,616 bytes、約21.3秒で合格した。旧warning／admission-stop reportは改善履歴として保持する。128pxは実行可能性のみで画像品質は未認定。証跡: [128px cleanup-qualified](evaluation/qwen-image-2512-mflux-streamed-2step-128px-cleanup-qualified-2026-09-24.json)
- `[Done]` 英語実promptを28層text encoderで処理し、private一回消費handoff後の別processで、実ノイズ→2 step・120 transformer block→FlowMatch Euler更新→VAE decodeを32×32で一体実行。28/28 encoder層、2/2 step、private残存0、有限`[1,3,1,32,32]`、pressure全件normal／thermal nominal。transformer/VAE processのpeak MLX 311,849,410 bytes／peak RSS 588,529,664 bytes、全体約50.0秒。32px・2 stepのため画像品質は未認定。証跡: [real-prompt streamed two-step](evaluation/qwen-image-2512-mflux-real-prompt-streamed-2step-32px-2026-09-24.json)
- `[Done]` 合成promptの256×256・2 stepを開始前available memory 14.28 GBから実行。2/2 step・120/120 block、有限`[1,3,1,256,256]`、pressure全件normal／thermal nominal、peak MLX 2,182,199,798 bytes、process peak RSS 581,320,704 bytes、約20.9秒で合格した。実prompt・十分なstep数・画像品質は未認定。証跡: [256px synthetic two-step](evaluation/qwen-image-2512-mflux-streamed-2step-256px-synthetic-2026-09-24.json)
- `[Done]` 英語実promptを28層encodeしてprivate handoffし、128×128で4 step・240/240 block、FlowMatch Euler更新、VAEまで一体実行。private残存0、有限`[1,3,1,128,128]`、pressure全件normal／thermal nominal、transformer/VAE processのpeak MLX 1,188,085,954 bytes／peak RSS 641,597,440 bytes。4 stepへの拡張実証であり、negative prompt／CFGと意味品質は未認定。証跡: [128px real-prompt four-step](evaluation/qwen-image-2512-mflux-real-prompt-streamed-4step-128px-2026-09-24.json)
- `[Done]` positive英語実promptと固定negative promptを同じ逐次28層encoderで別々に処理し、2つのprivate一回消費handoffを別processへ渡すtrue CFG経路。MFLUXと同じ`negative + guidance * (positive-negative)`およびcondition norm補正を適用し、guidance 4.0、32×32、2 stepで56/56 encoder層、positive/negative計240/240 block、VAE有限出力、handoff残存0、pressure normal／thermal nominalを確認。decoded標準偏差0.516、値域約2.22で数値上非退化。peak MLX 444,985,472 bytes、process peak RSS 622,952,448 bytes。32px・2 stepのため意味品質は未認定。証跡: [real-prompt true-CFG smoke](evaluation/qwen-image-2512-mflux-real-prompt-cfg-2step-32px-2026-09-24.json)
- `[Done]` true CFGを128×128・4 stepへ拡大。positive／negative promptの56 encoder層、4 step×positive/negative×60 block＝480/480 block、両private handoff cleanup、有限`[1,3,1,128,128]`、pressure全件normal／thermal nominal、約115.3秒で合格した。decoded標準偏差0.239、値域約2.27で数値上非退化。transformer/VAE processのpeak MLX 1,188,102,592 bytes、peak RSS 668,909,568 bytes。低解像度・4 stepのため視覚品質は未認定。証跡: [128px real-prompt true-CFG four-step](evaluation/qwen-image-2512-mflux-real-prompt-cfg-4step-128px-2026-09-24.json)
- `[Done]` 128×128・20 stepのtrue CFGと実PNG保存。positive／negative promptの56 encoder層、20 step×positive/negative×60 block＝2,400/2,400 block、両private handoff cleanup、有限`[1,3,1,128,128]`、全step memory pressure normal、最終thermal fair、約464.9秒で実行合格した。transformer/VAE processのpeak MLX 1,188,102,720 bytes、peak RSS 641,253,376 bytes。PNG SHA-256もreportと実fileで一致した。一方、実視ではpromptの「白いテーブル上の赤いリンゴ」を明確に識別できないため`image_quality_qualified=false`を維持する。証跡: [128px real-prompt true-CFG twenty-step](evaluation/qwen-image-2512-mflux-real-prompt-cfg-20step-128px-2026-09-24.json)、[generated PNG](evaluation/qwen-image-2512-real-prompt-cfg-20step-128px-2026-09-24.png)
- `[Done]` streamed transformer loaderをMLX標準`safetensors` readerと独立比較。固定`img_in`／`txt_norm`／`txt_in`／`time_text_embed`／RoPEと量子化blockを同一入力で通し、別shardのblock 0／30／59すべてでtext・image出力がbit-exact、最大絶対誤差0だった。pressure normal／thermal nominal、最大peak MLX 221,116,596 bytes、最大RSS 452,526,080 bytes。これによりexact-offset reader、4-bit layer構築、固定層、RoPE、先頭／中間／末尾blockは128px画質不良の第一原因から除外する。全60 blockの独立比較や画像品質認定には拡張しない。証跡: [block 0 parity](evaluation/qwen-image-2512-mflux-transformer-standard-reader-parity-block0-2026-09-24.json)、[block 30 parity](evaluation/qwen-image-2512-mflux-transformer-standard-reader-parity-block30-2026-09-24.json)、[block 59 parity](evaluation/qwen-image-2512-mflux-transformer-standard-reader-parity-block59-2026-09-24.json)
- `[Done]` streamed text encoderもMLX標準`safetensors` readerと独立比較。同一のBF16 hidden state、attention mask、Qwen RoPEをlayer 0／13／27へ入力し、3層すべての出力がbit-exact、最大絶対誤差0だった。層外の`embed_tokens.weight`／最終`norm.weight`／`rotary_emb.inv_freq`も全要素一致した。pressure normal／thermal nominal。層比較のpeak MLX 468,941,674 bytes／最大RSS 915,030,016 bytes、共通weight比較は一時的F32展開を含みpeak MLX 4,359,986,568 bytes／RSS 7,276,691,456 bytes。exact-offset BF16/F32 readerの先頭／中間／末尾層および全共通weightの不一致を第一原因から除外する。全28層の独立比較やprompt意味品質認定ではない。証跡: [layer 0 parity](evaluation/qwen-image-2512-mflux-text-layer-standard-reader-parity-layer0-2026-09-24.json)、[layer 13 parity](evaluation/qwen-image-2512-mflux-text-layer-standard-reader-parity-layer13-2026-09-24.json)、[layer 27 parity](evaluation/qwen-image-2512-mflux-text-layer-standard-reader-parity-layer27-2026-09-24.json)、[static weight parity](evaluation/qwen-image-2512-mflux-text-static-standard-reader-parity-2026-09-24.json)
- `[Done]` 256px本試験の安全なload前停止。開始前available 17.48 GBで親encoderを開始したが、positive／negative promptの56層encode後、生成childの14 GB admissionを満たさずtransformer weight load前に停止した。閾値を緩和せず再試行しない。従来はchild report生成前の停止を例外だけで失ったため、親qualificationを固定failure code、child return code、private handoff cleanupを最終JSONへ残してexit 2にするよう修正した。今回の停止は修正前の実行であり生成reportは存在しないため、成功証跡として扱わない
- `[Done]` text encoder residentをgenerationへ持ち越さないdisjoint-process orchestrator。親processはMLXをimportせず、positive encoder processを終了後にnegative encoder processを実行し、両終了後だけgeneration processを起動する。32px・2 step true CFGで2 encoder phase／56 layer、positive/negative計240 block、private handoff一回消費とroot cleanup、pressure normal、thermal nominalを確認した。既存inline親processの二重residentを避ける経路の実行証跡であり、256px品質認定ではない。証跡: [staged-process true-CFG smoke](evaluation/qwen-image-2512-mflux-staged-process-cfg-2step-32px-2026-09-24.json)
- `[Done]` staged orchestratorを160×160・20 stepへ一軸拡張。2 encoder phase／56 layer、2,400/2,400 block、有限`[1,3,1,160,160]`、全step pressure normal、最終thermal fair、handoff消費／root cleanup、PNG digest一致を確認した。約514.0秒、generation peak MLX 1,599,759,092 bytes、peak RSS 651,444,224 bytes、decoded標準偏差0.587。実視で赤いリンゴと白系の台面を明確に認識でき、固定prompt意味品質を合格とした。128px不合格から解像度だけを上げた改善証跡であり、256px memory stabilityの代替ではない。証跡: [execution](evaluation/qwen-image-2512-mflux-staged-real-prompt-cfg-20step-160px-2026-09-24.json)、[visual review](evaluation/qwen-image-2512-mflux-staged-real-prompt-cfg-20step-160px-visual-review-2026-09-24.json)、[PNG](evaluation/qwen-image-2512-staged-real-prompt-cfg-20step-160px-2026-09-24.png)
- `[Done]` staged orchestratorの256×256・実prompt・true CFG・20 step promotion。160px合格reportのartifact identity、20/20 all-normal、true CFG、cleanup、代表memory peakをstrict loaderで再検証し、次解像度を最大2倍、evidence-derived floorを最低10 GBに制限した。開始前available 11.62 GBから2 encoder phase／56 layer、2,400/2,400 block、有限`[1,3,1,256,256]`、全step pressure normal、最終thermal fair、handoff cleanup、PNG digest一致で合格。decoded標準偏差0.645、値域約2.18。実視品質も合格した。証跡: [execution](evaluation/qwen-image-2512-mflux-staged-real-prompt-cfg-20step-256px-2026-09-24.json)、[visual review](evaluation/qwen-image-2512-mflux-staged-real-prompt-cfg-20step-256px-visual-review-2026-09-24.json)、[PNG](evaluation/qwen-image-2512-staged-real-prompt-cfg-20step-256px-2026-09-24.png)
...
- `[Pending]` CPU/Core ML draft artifactによるmodel固有draft／verify実測profile生成。配置済みGemma 3 1B CPU draftは4B GPU verifierに対し約11.59倍遅く、より小さい同一tokenizer／vocabularyのdraftまたはGemma用Core ML artifactが未配置のため保留。必要artifact配置後、5%以上改善した合格profileだけを標準decode routeへ昇格する
...
121. `[Done]` Qwen-Image-2512-4bitのcomponent streaming/offloadと現行Macでの段階的memory-stability qualification。worker、readiness、3言語prompt encoder、disjoint encoder/generation process、true CFG＋256px 20-step denoising＋VAE／PNG、実視品質、標準readerとの代表層bit-exact比較まで完了。大容量Apple Siliconを必要とする一括512px経路のqualificationは別途`[Pending]`
...
252. `[Done]` vllm-mlxのmetadata-first memory accountingを参考に、MLX cache計測でshape／dtypeを優先。iterator探索で幅の広いcontainerの一括コピーを避け、scalar／重複参照も探索上限へ含め、不完全なsemantic snapshotを拒否・解放する。比較revision、回帰試験範囲、合成メモリ計測と実MLX検証の残課題は[比較記録](vllm-mlx-review.md)を参照

253. `[Done]` llama.cppのキャンセル時キュー回収を参考に、priority queueのキャンセル済みheap entryがconsumer不在時に無制限に残る問題を修正。待機数ゼロで即時clear、それ以外は閾値付きcompactionでpriority／FIFO／claim予約を保持しつつ内部metadataを制限。1万回の投入・キャンセルと既存dispatch競合を含む29テストを確認。[比較記録](llama-cpp-review.md)
