...
class ConcurrentRequestLoadTests(unittest.TestCase):
    def test_draining_rejects_new_work_and_waits_for_active_request(self) -> None:
        service = BlockingRuntimeService(1)
        server = create_server("127.0.0.1", 0, service, max_concurrent_requests=2)
        server_thread = threading.Thread(target=server.serve_forever, daemon=True)
        server_thread.start()
        base_url = f"http://127.0.0.1:{server.server_port}"
        worker = threading.Thread(
            target=lambda: urllib.request.urlopen(base_url + "/health", timeout=4).close()
        )
        try:
            worker.start()
            ...
            self.assertEqual(
                json.load(raised.exception)["error"]["code"], "server_draining"
            )
            self.assertFalse(server.wait_for_drain(0.01))
            service.release.set()
            self.assertTrue(server.wait_for_drain(2))
