...
def _validate(instance: Any, schema: dict[str, Any], path: str) -> None:
    ...
    if isinstance(instance, list):
        ...
        if minimum_items is not None and len(instance) < minimum_items:
            raise SchemaValidationError(f"{path}: array has fewer than {minimum_items} items")
        if maximum_items is not None and len(instance) > maximum_items:
            raise SchemaValidationError(f"{path}: array has more than {maximum_items} items")
    ...
    if isinstance(instance, str):
        ...
        if maximum_length is not None and len(instance) > maximum_length:
            raise SchemaValidationError(f"{path}: string is longer than {maximum_length}")
