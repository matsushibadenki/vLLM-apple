...
def build_dry_run_plan(
    ...
    *,
    ...
    adapter_capability = builtin_adapter_registry().detect(model).adapters[0]
    candidates = tuple(
        _candidate(
            bits,
            source.weights_bytes,
            resource_budget,
            performance_profile,
            adapter_capability.executable and bits in adapter_capability.weight_bits,
        )
...
def _candidate(
    ...
    adapter_executable: bool,
) -> OptimizationCandidate:
    # Packed weights plus conservative 10% metadata/alignment overhead.
    output_bytes = math.ceil(weights_bytes * bits / 16 * 1.10)
    # Reserve two output-sized work areas plus fixed metadata space so adapters
    # cannot silently overcommit disk.
    ...
    reasons = [] if adapter_executable else ["adapter_unavailable"]
    ...
    return OptimizationCandidate(
        ...
        executable=adapter_executable and within_budget,
