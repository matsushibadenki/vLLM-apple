...
optimizeButton?.addEventListener('click', () => {
  ...
  if (optimizeLabel) optimizeLabel.textContent = 'Checking your Mac…';
  if (memoryStatus) memoryStatus.textContent = 'Measuring memory';
  window.setTimeout(() => {
    optimizeButton.classList.add('done');
    optimizeButton.disabled = false;
    if (optimizeLabel) optimizeLabel.textContent = 'Optimized';
    if (memoryStatus) memoryStatus.textContent = 'Safe plan applied';
  }, 900);
});
